...
def visualizeTruss(X, E, C, F, annotate=False, axisLimits=None):
    """
    ...
    """
    lc = mc.LineCollection(X[E], linewidths=3, color='dodgerblue', zorder=0)
    ...
    plt.scatter(X[freeNodes, 0], X[freeNodes, 1], zorder=1, color='k', s=60)
    plt.scatter(X[C, 0], X[C, 1], zorder=1, color='orange', s=60)
    plt.scatter(X[forceNodes, 0], X[forceNodes, 1], zorder=1, color='red', s=60)
    
    plt.quiver(X[:, 0], X[:, 1], F[:, 0], F[:, 1], width=0.005, scale=10, zorder=0, color='red')
